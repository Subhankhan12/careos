...
> | ~~3~~ | ~~**Invisible refusals**~~ — **CLOSED by QA-FIX.11a (`51017e2`)**: `P8-H1`, `P9-H3`, `P10-H2` FIXED across 30 sites on 17 pages. `P10-M1` is **PARTLY** fixed (approval queue only; its other 24 pages need a per-page replace-or-duplicate decision — design work, not adoption) and **`QF11a-M1` was recorded**: Clinical's own pages render no error bag either. | ~~4~~ | ~~**YES, STRONG — `resources/js/Components/RefusalNotice.vue`**~~ (D-210, QA-FIX.6c; re-applied by QA-FIX.7c/D-213). Already at 12 call sites across Surgery, ED and Radiology. Remaining modules: **Lab, Radiology (beyond the one site QA-FIX.8b added for its own new refusal), Hospital (11 refusal sites, 0 renderers), Governance.** `P10-H2` additionally needs a catch widened — `BookingConflictException` is not an `AiCoreException`, so a domain refusal escapes as a 500. |
...
> | ~~6~~ | ~~**Attribution recorded but never surfaced, or resolved by convenience**~~ — **CLOSED by QA-FIX.11b (`18786eb`)**. `P9-H6` was a genuine attribution defect (latent in the seeded data; 32 historical accruals, **0 miscredited**); `P8-H3` was a **display** defect and its fix renders a name without touching a write. | ~~2~~ | ~~**YES — D-216/D-220.**~~ `P9-H6`'s remedy already exists and is already named in D-220: **`SystemActorResolver::forPermission()`** (`Modules/Platform/src/Services/SystemActorResolver.php:45`), which every other scheduled command already uses. `P8-H3` is a rendering task: the actor is recorded correctly everywhere and named on no surface. |
...
>
> **STATE AFTER QA-FIX.11 (`28f64a1`): families 3 and 6 are CLOSED. 29 open HIGH remain**, re-derived by
> counting the artifact: family **1** (16, unchanged — still half the open HIGHs and still needing triage
> before scheduling), family **2** (5), family **4** (3, precedent D-221/D-222 — the cheapest next gate),
> family **5** (3, precedent D-091), family **7** (**1** — `P4-H2`, the last create-then-associate instance),
> plus **`P10-H1`** (a refused approval still appends a stale `ai_interaction.approved` row — a
> recording-ORDER defect, named inside QA-FIX.10c's own test). **`QF11a-M1` and `QF10a-H1` are the two
> findings this programme's fix gates recorded rather than widened into.**
>
> **Recommended order now: family 4** (3 findings, strong precedent, all honesty defects on shipped
> surfaces), then **7** (one finding, proven remedy, closes a six-instance pattern for good), then **5**.
> Leave **1** for last and triage it first — it is not one job.
>
