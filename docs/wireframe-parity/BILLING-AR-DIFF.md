...
| Top-overdue balances / age / stage | ✅ (data) / 🚧 (rollup) | Balances read from stored `open_balance_minor` + folded `DunningEvent`s (no money summing — fence-safe). But a per-ACCOUNT "top N by balance" rollup is not an engine aggregate yet (BILLAR.P7). |
...
| BILLAR.P7+ | account-level overdue rollup + AR Account Detail (top-overdue drill) · payer-dimension enrichment (accident/social + insurer entity) · PDF export | pending |
