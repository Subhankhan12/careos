...
| Collectible / **Net collection rate** | ✅ (BILLAR.P3/P6) | `netCollectionRate` — engine collectible (charges − contractual) + ratio; P6 grid displays it, honest null → "—". |
| **DSO** | ✅ (BILLAR.P3/P6) | `daysSalesOutstanding` — engine; P6 grid displays it, honest null → "—". |
| **AR roll-forward** (opening/charges/collections/adjustments/write-offs/closing) | ✅ (BILLAR.P1/P2/P6) | The tying opening→closing bridge is engine-computed (`arRollForward`, δ=0) and DISPLAYED on the P6 grid with the tie badge; the page never computes the bridge or subtracts lines client-side. |
| **By-payer amounts** | ✅ (BILLAR.P4/P6) | `arByPayer` groups over the real `payer_type` (groups tie to total δ=0); P6 grid displays the split + tie badge. |
| Top-overdue balances / age / stage | ✅ (BILLAR.P7) | `MetricsService::topOverdueAccounts` — per-account overdue rollup: balances from `open_balance_minor`, ages from due-date vs as-of (aging day math), the STAGE = the real `max(DunningEvent.level)`, ordered most-overdue-first; the rollup ties to its invoices + `overdueBalanceMinor` (δ=0). Displayed on the P6 grid; each row drills to AR Account Detail. No page-side money math. |
| Charged-vs-collected trend points | ✅ (BILLAR.P5/P6) | `MetricsService::chargedVsCollectedTrend` — engine time-series, buckets partition the range (Σ === totals δ=0), same shared helpers. P6 grid renders it as the column chart; no page-side math. |
...
| **BILLAR.P6** | The Billing & AR **management-report grid** (`/billing/report`, `BillingReportController`, `Billing/Report.vue`): consolidates the aging/reporting/dunning AR content into one grid that DISPLAYS the P1–P5 engine figures (headline / stat cards / aging / roll-forward / by-payer / DSO+rate / trend). The Vue computes NO money; the period switcher (Week/Month/Quarter/YTD + Compare) re-parameterizes the engine (server recomputes); CSV export streams the engine figures. Keeps the live aging/reporting/dunning routes working (links, no redirect). billing.view + tenant-scoped | **RESOLVED** |
| **BILLAR.P7** | The **top-overdue accounts table** on `/billing/report` + the account rollup + the drill to AR Account Detail (`MetricsService::topOverdueAccounts`, `AccountDetailController`, `Billing/AccountDetail.vue`): overdue accounts (account · invoices · age · dunning STAGE · balance) ordered most-overdue-first; ages/balances are engine figures, the STAGE is the REAL persisted state-machine level (max `DunningEvent.level`), the account rollup ties to its invoices and the grand total ties to `overdueBalanceMinor` (δ=0); each row drills to `/billing/accounts/{account}`. billing.view + tenant-scoped; no page-side money math | **RESOLVED** |

**BILLING & AR wireframe-parity is COMPLETE** — P1 write-offs/adjustments → P2 roll-forward → P3 DSO/rate → P4 by-payer → P5 trend → P6 report grid → P7 top-overdue + drill. Every wireframe item is RESOLVED (or a flagged, honest gap: the finer Swiss payer taxonomy + insurer entity, and PDF export). The drill wires **AR Account Detail** as the next page's target (its full per-invoice ledger is the next gate).
