...
## STATUS: BUILD COMPLETE + AUDITED (3 audits, ZERO must-fix) · EIGHT VERTICALS · DEMOABLE HOSPITAL TENANT · CURRENT STAGE = DEPLOYMENT · WIREFRAME-PARITY PASS (4 PAGES DONE: Admin Settings · Approval Queue P1→P7 · Branches P1→P5 · Agent&Tool Config P1→P6 COMPLETE · Allergy Alert safe-part [ALLERGY.P1])
...
> Pest **1093 passed** (+6 from ALLERGY.P1), PHPStan L5 `[OK]`, Pint clean; `composer test:smoke`
> green; CI green on MySQL 8; the FIX.5 route-smoke guards against request-time 500s. (Latest commit: ALLERGY.P1.)
>
...
>   exist) is UNCHANGED. **BRANCH.P2** added the real `is_primary` default-branch flag (the "main" badge) with the
>   **exactly-one-primary-per-tenant invariant** (first branch primary via a model hook; atomic set-primary;
...
>   narrows the callable set but never grants past a tool's ceiling (resolver caps at runtime, P1). **AGENT.P5**
>   filled the per-agent hero metrics + the action-ledger tab from REAL records only (the ai_interactions ledger +
>   ApprovalQueue outcomes): drafts today / approved-as-is % (honest "—" when nothing resolved) / fence-refused 7d,
>   and a read-only view of the immutable ledger (fence_refused system-attributed w/ reason); attribution via a new
>   AgentRegistry alias map; a real fence_refused seeded through the real path. **AGENT.P6 (COMPLETE)** added the
>   rate/timing limits the AgentRuntime actually reads (max drafts/hour + quiet hours → the agent stops/defers; a
>   limit only narrows), the always-on uncertainty escalation (the real clinician-attention hand-off — locked-on,
>   no disable path), and the new-agent wizard (a P1 governed container of a real kind, capped from birth; forged
>   above-ceiling clamped). **AGENT & TOOL CONFIG wireframe-parity COMPLETE (P1→P6)** — the FOURTH page of the pass
>   (Admin Settings + Approval Queue + Branches + Agent & Tool Config). Next parity page: Allergy Alert (safe part).
...
- **State:** **BACKEND FEATURE-COMPLETE** — the shared platform (Phases 0/A/B/C/D/E/F/G + Phase-P hardening
  ...
  - **EMERGENCY DEPARTMENT / ED — STARTED (Hospital Phase 6, ED.G1 = the foundation).** A peer `Modules\ED`
    ...
    **ED.G5 (built):** **disposition + the ED→ADT handoff** (the signature reuse) — closing a visit
    (admit/discharge/transfer) via the G1 `awaiting_disposition → dispositioned` transition. **ADMIT reuses the
    EXISTING, concurrency-safe, atomic `AdmissionService::admit`** to create a Phase-1 inpatient `Stay`
    (`admission_type=emergency`); the cross-vertical composition is APP-LAYER (`app/Services/EdDispositionService`
    — so `Modules\ED` stays independent of `Modules\Hospital`; soft `stay_id` link). Atomic (admit + disposition
    in one tx — a failure rolls back both); reused bed-safety (`BedNotAvailableException` on an occupied bed);
...
- **HOSPITAL.G3 — the ward board: the live bed-occupancy cockpit (the first inpatient UI; D-115).**
  PRESENTATIONAL over G1/G2 (P0D.GU): `WardBoardController::show` (GET `/hospital/wards`, gate `patient.view`)
  renders `Hospital/WardBoard.vue` — each active ward → its beds by **housekeeping status** (free/occupied/
  cleaning/blocked), the current patient + `admitted_at` per occupied bed, and a plain **occupancy count**.
  (1) **Reuses the day-board tile/status idiom for LAYOUT, but the data is beds/stays (continuous occupancy)
  — never the scheduling slot engine.** (2) **Actions surfaced from the board go through the EXISTING G1/G2
  services** — admit/transfer/discharge POST the G2 routes (**admit-from-board uses the proven claim, atomic
  — tested**), bed status via a new `setBedStatus` (bed.manage). (3) **FENCE:** operational only — status is
  housekeeping (its colour a housekeeping state, not acuity), LOS is elapsed time, the summary is a count; NO
  acuity/severity/risk/priority field (recursive-payload fence test). (4) Read = `patient.view` (ward nurses
...
- **HOSPITAL.G5 — nursing shift handover (SBAR, nurse-authored, record-not-judge; D-117).** DISCOVERY: a
  handover is a **NET-NEW structured SBAR artifact, NOT a `ClinicalNote` reuse** (SBAR ≠ SOAP; shift metadata;
  stay-scoped not encounter-scoped) — it reuses the platform PATTERNS (append-only + DB triggers, audit,
  LogsReads, note/chart UI idioms). `handovers` (append-only): stay + outgoing nurse + shift {day/evening/
  night} + the SBAR text (situation [required] / background / assessment / recommendation — all nurse-authored)
  + reason. `HandoverService` record (gate `note.write` — the nursing roles hold it, NO new permission) +
  history; `HandoverController` (string-id): show (patient.view, read-logged) renders `Hospital/Handover.vue`,
  store (note.write); write audited via an app-layer `Handover::created` hook. **FENCE (record-not-judge):**
  the SBAR fields are the nurse's words — `assessment` is the nurse's OWN written assessment, never a computed
...
- **SURGERY.G2 — the case LIFECYCLE + op documentation + anesthetist-assigned ASA (D-126).** Spans the map's
  §2.2 (lifecycle) AND §2.3 (op-notes/ASA). **(1)** The G1 `SurgicalCase` gains a **legal-only state machine**
  (`TRANSITIONS` + `canTransition`; scheduled → pre_op → in_progress → completed → post_op, + cancelled from a
  not-yet-started case); `SurgicalCaseService::transition` (gate `surgery.manage`) asserts legal (else
  `invalidTransition`) → forceFills status + a **factual per-phase timestamp** + appends a `SurgicalCaseEvent`.
  **(2)** `surgical_case_events` — APPEND-ONLY (model guards + DB triggers, the `medication_order_events`
  recipe); audited app-layer `surgical_case.<event_type>`. **(3)** the surgical **team**
  (`surgical_case_team_members`). **(4) OP DOCUMENTATION reuses Clinical, `Encounter` UNMODIFIED** (the
  `ward_rounds`/`BedsideChartService` precedent): a `surgical_case_encounters` link; `startNote` opens a
  `TYPE_PROCEDURE` `Encounter` via the EXISTING `EncounterService::open`, drafts a `ClinicalNote`, then
  **CLOSES the encounter** so the one-open-per-practitioner invariant is preserved for other verticals
  (tested); the surgeon signs/amends via the EXISTING note editor. **(5) ASA/Mallampati are
  ANESTHETIST-ASSIGNED recorded facts** (closed sets I–VI / I–IV + provenance) — **the system computes NO
  surgical-risk score/prediction** (schema fence + a module grep); the intra-op device-data feed stays a
  partner stub (noted, not built). **(6) UI:** `SurgicalCaseController` + `Surgery/CaseBoard.vue` +
  `Surgery/Case.vue` + i18n (the case prop is `surgicalCase`, not the reserved `case`). No charge (billing is
  a later gate). No existing behavior test modified; **Encounter's invariant + Clinical's suite + the eval +
...
- **SURGERY.G4 — consumables + implant tracking (D-128).** MIRRORS the pharmacy G4 inventory recipe (Surgery
  can't import the peer Pharmacy vertical → COPIED with Surgery-owned tables): `surgical_items` catalog (+
  `is_implant` flag) → `surgical_item_stocks` (on-hand under a `FOR UPDATE` lock, `isBelowThreshold` factual) →
  append-only `surgical_stock_movements`. `case_item_usages` (append-only) — `recordUsage` (gate `note.write`)
  does the ATOMIC decrement (`lockOnHand → assert → usage → decrement → 'used' movement`); **concurrency-safe,
  hammer-proven** (8 processes race the last unit → 1 winner, on_hand=0). NET-NEW `implant_placements`
  (lot/serial/UDI → patient, indexed for recalls); `placeImplant` (gate `note.write`) decrements + records
  atomically; **the recall lookup `patientsForLot(lot|udi)` is a FACTUAL query** (the same lot in 2 patients →
  both), never a device verdict. Stock admin `surgery.manage`. FENCE: operational/traceability facts — no
  verdict/safe/recall_status/grade column (schema fence) + no `verifyDevice`/`recallStatus` method (grep). UI:
  `SurgicalInventoryController` + `Surgery/Inventory.vue`; `CaseSuppliesController` + `Surgery/CaseSupplies.vue`.
  No charge. No existing behavior test modified; eval + G1–G3 + reconciliation/fence/immutability green; FIX.5
